...
/** fetch with timeout + retry on 429/5xx */
async function fetchWithRetry(url: string | URL, init: RequestInit, retries = MAX_RETRIES): Promise<Response> {
...
export async function storagePut(
  ...
  // 1. Get presigned PUT URL from Forge (with retry)
  ...
  // 2. PUT file directly to S3 (with retry)
  const blob =
    typeof data === "string"
      ? new Blob([data], { type: contentType })
      : new Blob([data as any], { type: contentType });

  const uploadResp = await fetchWithRetry(s3Url, {
    method: "PUT",
